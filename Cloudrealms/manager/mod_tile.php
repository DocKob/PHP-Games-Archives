...
<b>Modify tile for row <? echo $rowx; ?> and column <? echo $column; ?></b>
...
<form method='post' action='<? echo $_SERVER[REQUEST_URI]; ?>'>
<?
echo "Teleport too: <select name='portal'>";
echo "<option>Select</option>";
$getLocationsE = mysql_query("SELECT * FROM locations");
while($row = mysql_fetch_array($getLocationsE))
{
	echo "<option value='$row[name]'>$row[display_name]</option>";
}
echo "</select>";
?>
<input type="submit" name="set_portal" value="Go!" >
</form>
<hr>
Place an NPC:
<form method='post' action='<? echo $_SERVER[REQUEST_URI]; ?>'>
...
<hr>
<form method='post' action='<? echo $_SERVER[REQUEST_URI]; ?>'>
<input type='submit' value='Clear tile' name='clear_tile' />
</form>
<?
if($_POST[upload])
{
	...
	mysql_query("UPDATE tiles SET bg = '$new_tile' WHERE id = '$tileid'") or die (mysql_error());
	echo "Tile added successfully!";
	echo "<script>setTimeout('self.close();',1000)</script>";
} else if($_POST[select]){
	mysql_query("UPDATE tiles SET bg = '$_POST[from_tiles]' WHERE id = '$tileid'") or die (mysql_error());
	echo "Tile updated successfully!";
	echo "<script>setTimeout('self.close();',1000)</script>";
} else if($_POST[set_portal]){
	mysql_query("UPDATE tiles SET portal = '$_POST[portal]' WHERE id = '$tileid'") or die (mysql_error());
	echo "Tile updated successfully!";
	echo "<script>setTimeout('self.close();',1000)</script>";
	...
	echo "<script>setTimeout('self.close();',1000)</script>";
} else if($_POST[clear_tile]){
	mysql_query("DELETE FROM tiles WHERE id = '$tileid'");
	echo "Tile cleared!";
	echo "<script>setTimeout('self.close();',1000)</script>";
